...
public class HTTPAsk {
	...
	public static void main(String[] args) throws IOException, URISyntaxException {
		if (args.length != 1) {
			log("Usage: java HTTPAsk <port>");
			System.exit(1);
		}

		HTTPAsk ask = new HTTPAsk();
 
		int port = Integer.parseInt(args[0]);
		ask.open(port);
	}

	private void open(int port) throws IOException, URISyntaxException {
		this.port = port;
	...
	private static class Options {
		...
		public Options(String host, int port, boolean shutdown, 
						Integer timeout, Integer limit, byte[] data) {
	...
	private Optional<Options> getOptions(Map<String, String> params) {
	...
	private void writeResponse(HTTPResponse  httResponse, OutputStream out) throws IOException {
		...
		String response = 
					"HTTP/1.0 " + httResponse.statusCode + "\r\n" +
	...
	private Map<String, String> getParams(URI uri){
		...
		if(query.isBlank()) return queryPairs;
	...
	private Optional<HttpRequest> parseRequest(InputStream in){
		...
		URL url;
		try {
			url = new URL(protocol, host, port, uri);
